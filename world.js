var createWorldCreator = function() {
	...
	creator.spawnUserRandomly = function(floorCount, floorHeight, floors, fireExit) {
		var user = creator.createRandomUser();
		user.moveTo(105 + random(40), 0);
		var currentFloor = !fireExit && random(1) === 0 ? 0 : random(floorCount - 2) + 1;
	...
	creator.createWorld = function(options) {
		...
		world.update = function(dt) {
			...
			while (elapsedSinceSpawn > 1.0 / options.spawnRate) {
				...
				registerUser(
					creator.spawnUserRandomly(options.floorCount, world.floorHeight, world.floors, options.fireExit),
				);
			}
...
var createWorldController = function(dtMax) {
	...
	controller.start = function(world, codeObj, animationFrameRequester, autoStart) {
		reSeed("082e25cd-7e3e-4a0d-ba69-06fa96c40473");
		controller.isPaused = true;
		var lastT = null;
